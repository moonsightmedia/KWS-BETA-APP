...
export async function resumableUpload(
  ...
  const { sessionId, sectorId, onProgress, abortSignal, authToken: providedAuthToken } = options;
  ...
  try {
    // CRITICAL: Fall back to a fresh session lookup if no token was provided
    // (e.g. when a long-running upload's original token becomes unavailable).
    ...
    // If every chunk is already present (e.g. the client crashed right before receiving
    // the final response), force a retry of the last chunk so the server can respond with the URL.
